    ...
    func process(_ result: Result<Response, MoyaError>, target: TargetType) -> Result<Response, MoyaError> {
        ...
        case .success(let response):
            ...
            if newResponse.statusCode == successCode {
                ...
                let errorMsg = "net request error"
                let nsError: NSError = NSError(domain: "net.error", code: newResponse.statusCode, userInfo: [NSLocalizedDescriptionKey: errorMsg, NSLocalizedFailureReasonErrorKey: errorMsg])
                return .failure(MoyaError.underlying(nsError, newResponse))
            }
        case .failure(_):
            break
        }
